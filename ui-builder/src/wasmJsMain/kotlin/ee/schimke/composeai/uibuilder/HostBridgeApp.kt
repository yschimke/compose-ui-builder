...
internal fun openHostDesign(messageJson: String, generation: Int): HostOpenedDesign {
  ...
  val document =
    if (message.document.isNotBlank()) {
      ...
      // The host sends this archive's packaged capability documents, so the seed is written in
      // their vocabulary, as the desktop app's and the IntelliJ plugin's are.
      val vocabulary = UiBuilderNewDesignSeed.Vocabulary.PACKAGED
      require(seed.templateId in UiBuilderNewDesignSeed.templateIds(systemId, vocabulary)) {
