...
internal actual fun WearCanvasDatePicker(initialDate: String, type: String, modifier: Modifier) {
  DatePicker(
    initialDate =
      runCatching { LocalDate.parse(initialDate) }.getOrElse { LocalDate.of(2026, 1, 1) },
    onDatePicked = {},
    modifier = modifier,
    datePickerType =
      when (type) {
...
internal actual fun WearCanvasTimePicker(initialTime: String, type: String, modifier: Modifier) {
  TimePicker(
    initialTime = runCatching { LocalTime.parse(initialTime) }.getOrElse { LocalTime.of(10, 10) },
    onTimePicked = {},
    modifier = modifier,
    timePickerType =
      when (type) {
