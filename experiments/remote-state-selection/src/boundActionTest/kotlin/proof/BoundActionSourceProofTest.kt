...
import proof.boundactions.remote.RemoteRepeatedContent

@RunWith(RobolectricTestRunner::class)
@Config(sdk = [35])
@GraphicsMode(GraphicsMode.Mode.NATIVE)
class BoundActionSourceProofTest {
  ...
  private fun verify(density: Int) = runTest {
    ...
    val output = File("build/bound-action-evidence").apply { mkdirs() }
    ...
    for ((index, step) in cases.withIndex()) {
      ...
      save("remote-$index-row-$row-$name")
    }
  }
}
