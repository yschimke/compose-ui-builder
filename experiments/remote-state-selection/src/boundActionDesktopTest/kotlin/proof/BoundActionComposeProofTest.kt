...
import kotlin.test.*
import proof.boundactions.compose.ComposeRepeatedContent

@OptIn(ExperimentalTestApi::class)
class BoundActionComposeProofTest {
  @Test fun boundCallbacksAtDensityOne() = verify(1)

  @Test fun boundCallbacksAtDensityTwo() = verify(2)
  ...
  private fun verify(density: Int) =
    runDesktopComposeUiTest(width = 100 * density, height = 120 * density) {
      ...
      val output =
        File(root, "experiments/remote-state-selection/build/bound-action-evidence").apply {
          mkdirs()
        }
      ...
      val cases =
        listOf(
          Triple(2, "select", 0xff0000ff.toInt()),
          Triple(0, "select", 0xffff0000.toInt()),
          Triple(1, "select", 0xff00ff00.toInt()),
          Triple(2, "reset", 0xffff0000.toInt()),
          Triple(2, "select", 0xff0000ff.toInt()),
          Triple(0, "reset", 0xffff0000.toInt()),
          Triple(1, "select", 0xff00ff00.toInt()),
          Triple(1, "reset", 0xffff0000.toInt()),
        )
      for ((index, step) in cases.withIndex()) {
        val (row, name, color) = step
        val x = if (name == "select") 12 else 28 + row * 8
        frame.performTouchInput {
          click(Offset(x.toFloat() * density, (10f + row * 24) * density))
        }
        val image = capture("compose-$index-row-$row-$name")
        assertEquals(
          color,
          image.getRGB(30 * density, 78 * density),
          "$name from row $row retains its own bound value",
        )
      }
    }
}
